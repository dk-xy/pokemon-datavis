...
function makeChordChart(svgTarget, resTarget) {
  ...
  let innerBars = svg.datum(res).append("g").selectAll("path").data(function (d) {
    return d;
  }).enter().append("path").attr('class', 'innerArcs').on("mouseover", onMouseOver).on("mouseout", onMouseOut);
...
makeStatsMiniBoxPlot(stats.steel, 'water'); // creation tableau simplifié
//top 10
//mise en place légendes

makeLegend(typeArray, colors); //0 - Uber--------------------------------------------------------------
