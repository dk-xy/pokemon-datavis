...
import { domOn, domForEach } from './domManipulator'; //Préparation UI--------------------------------
//console.log("hello")

console.log(pokemon);
window.addEventListener('hashchange', displaySection);
window.onload = displaySection; //PREPARATION DATA--------------------------------------------

const typeArray = ["bug", "dark", "dragon", "electric", "fairy", "fire", "fighting", "flying", "grass", "ghost", "ground", "ice", "normal", "poison", "psychic", "rock", "steel", "water"]; //Préparation data matricielles ^o^--------------------------------
//creation de la matrice des types avec value à 0

function sortTypes(theArray) {
  let lineMatrice = new Array(18); //console.log(theArray)
...
function makeMatrice(array) {
  //1 - bug----------------------------------------
  const bugArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "bug"); //console.log(bugArray)

  let bug_matrice_done = sortTypes(bugArray); //2 - dark

  const darkArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "dark");
  let dark_matrice_done = sortTypes(darkArray); //3 - dragon

  const dragonArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "dragon");
  let dragon_matrice_done = sortTypes(dragonArray); //4 - electric

  const electricArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "electric");
  let electric_matrice_done = sortTypes(electricArray); //5 - fairy

  const fairyArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fairy");
  let fairy_matrice_done = sortTypes(fairyArray); //6 - fire

  const fireArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fire");
  let fire_matrice_done = sortTypes(fireArray); //7 - fighting

  const fightArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fighting");
  let fight_matrice_done = sortTypes(fightArray); //8 - flying

  const flyArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "flying");
  let fly_matrice_done = sortTypes(flyArray); //9 - grass

  const grassArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "grass");
  let grass_matrice_done = sortTypes(grassArray); //10 - ghost

  const ghostArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "ghost");
  let ghost_matrice_done = sortTypes(ghostArray); //11 - ground

  const groundArray = array.filter(pkmn => pkmn.type1 == "ground");
  let ground_matrice_done = sortTypes(groundArray); //12 - ice

  const iceArray = array.filter(pkmn => pkmn.type1 == "ice");
  let ice_matrice_done = sortTypes(iceArray); //13 - normal

  const normalArray = array.filter(pkmn => pkmn.type1 == "normal");
  let normal_matrice_done = sortTypes(iceArray); //14 - poison

  const poisonArray = array.filter(pkmn => pkmn.type1 == "poison");
  let poison_matrice_done = sortTypes(poisonArray); //15 - water

  const waterArray = array.filter(pkmn => pkmn.type1 == "psychic");
  let water_matrice_done = sortTypes(waterArray); //16 - psychic

  const psyArray = array.filter(pkmn => pkmn.type1 == "rock");
  let psychic_matrice_done = sortTypes(psyArray); //17 - steel

  const steelArray = array.filter(pkmn => pkmn.type1 == "steel");
  let steel_matrice_done = sortTypes(steelArray); //18 - rock

  const rockArray = array.filter(pkmn => pkmn.type1 == "water");
  let rock_matrice_done = sortTypes(rockArray); //19 - none
  //console.log(waterArray)
  //console.log(bugArray)
  //console.log(water_matrice_done)

  let matrix = [[...bug_matrice_done], [...dark_matrice_done], [...dragon_matrice_done], [...electric_matrice_done], [...fairy_matrice_done], [...fire_matrice_done], [...fight_matrice_done], [...fly_matrice_done], [...grass_matrice_done], [...ghost_matrice_done], [...ground_matrice_done], [...ice_matrice_done], [...normal_matrice_done], [...poison_matrice_done], [...water_matrice_done], [...psychic_matrice_done], [...steel_matrice_done], [...rock_matrice_done]];
  return matrix;
}

let matrix = makeMatrice(pokemon); //console.log(matrix)
...
function makeStatsByType(array) {
  let hp = array.map(function (d) {
    return d.hp;
  });
  let atk = array.map(function (d) {
    return d.attack;
  });
  let def = array.map(function (d) {
    return d.defense;
  });
  let sp_atk = array.map(function (d) {
    return d.sp_attack;
  });
  let sp_def = array.map(function (d) {
    return d.sp_defense;
  });
  let spd = array.map(function (d) {
    return d.speed;
  });
  const statsForType = {
    "HP": hp,
...
function makeStatsArray(array) {
  ...
  let itemBug = makeStatsByType(bugArray); //2 - dark

  const darkArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "dark");
  let itemDark = makeStatsByType(darkArray); //3 - dragon

  const dragonArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "dragon");
  let itemDragon = makeStatsByType(dragonArray); //4 - electric

  const electricArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "electric");
  let itemElectric = makeStatsByType(electricArray); //5 - fairy

  const fairyArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fairy");
  let itemFairy = makeStatsByType(fairyArray); //6 - fire

  const fireArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fire");
  let itemFire = makeStatsByType(fireArray); //7 - fighting

  const fightArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "fighting");
  let itemFight = makeStatsByType(fightArray); //8 - flying

  const flyArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "flying");
  let itemFly = makeStatsByType(flyArray); //9 - grass

  const grassArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "grass");
  let itemGrass = makeStatsByType(grassArray); //10 - ghost

  const ghostArray = array.filter(pkmn => pkmn.type1.toLowerCase() == "ghost");
  let itemGhost = makeStatsByType(ghostArray); //11 - ground

  const groundArray = array.filter(pkmn => pkmn.type1 == "ground");
  let itemGround = makeStatsByType(groundArray); //12 - ice

  const iceArray = array.filter(pkmn => pkmn.type1 == "ice");
  let itemIce = makeStatsByType(iceArray); //13 - normal

  const normalArray = array.filter(pkmn => pkmn.type1 == "normal");
  let itemNormal = makeStatsByType(iceArray); //14 - poison

  const poisonArray = array.filter(pkmn => pkmn.type1 == "poison");
  let itemPoison = makeStatsByType(poisonArray); //15 - water

  const waterArray = array.filter(pkmn => pkmn.type1 == "psychic");
  let itemWater = makeStatsByType(waterArray); //16 - psychic

  const psyArray = array.filter(pkmn => pkmn.type1 == "rock");
  let itemPsychic = makeStatsByType(psyArray); //17 - steel

  const steelArray = array.filter(pkmn => pkmn.type1 == "steel");
  let itemSteel = makeStatsByType(steelArray); //18 - rock
...
let svg = d3.select("#types-chart").append("svg").attr("width", 1000).attr("height", 800).append("g").attr("transform", "translate(600,400) scale(1.2,1.2)"); //calcul de la matrice

let res = d3.chord().padAngle(0.03).sortSubgroups(d3.descending).sortChords(d3.descending)(matrix); //console.log(smogon)
...
function makeChordChart(svgTarget, resTarget, typeArray, matrix) {
  ...
  let outerGroups = svg.datum(res).append("g").selectAll("g").data(function (d) {
    return d.groups;
  }).enter().append("g");
  let outerBars = outerGroups.attr('class', "group").attr('type', function (d) {
    return typeArray[d.index];
  }).append("path").attr("id", d => d.type1).style("fill", function (d, i) {
    return colors[i];
  }).style("stroke", function (d, i) {
    return colors[i];
  }).attr("d", d3.arc().innerRadius(200).outerRadius(210)); //append  des elements g

  let outerText = outerBars.data(res.groups).enter().append("svg:g").attr("class", function (d) {
    return "group " + typeArray[d.index];
  }).append("svg:textPath").text(function (d) {
    return typeArray[d.index];
  });
  let textGroups = d3.selectAll('g.group').append("text").each(function (d) {
    d.angle = (d.startAngle + d.endAngle) / 2;
  }).attr("dy", ".05em").attr("class", "titles").attr("text-anchor", function (d) {
    return d.angle > Math.PI ? "end" : null;
  }).attr("transform", function (d) {
    return "rotate(" + (d.angle * 180 / Math.PI - 90) + ")" + "translate(" + (outerRadius - 100) + ")" + (d.angle > Math.PI ? "rotate(180)" : "");
  }).text(function (d, i) {
    return typeArray[i];
  }); // Ajout des liens entre les groupes-------

  let innerBars = svg.datum(res).append("g").selectAll("path").data(function (d) {
    return d;
  }).enter().append("path").attr('class', function (d) {
    return typeArray[d.source.index] + "Arc " + typeArray[d.target.index] + "Sec" + ' innerArcs';
  }).on("mouseover", onMouseOver).on("mouseout", onMouseOut);
  let ribbons = innerBars.attr("d", d3.ribbon().radius(200)).style("fill", function (d) {
  ...
  function onMouseOver(selected) {
    ...
    let colorFill = "fill:" + color; //set des bars a opacité diff sur hover

    innerBars.style("opacity", 0.1);
    ...
    domForEach('.innerArcs', evt => {
      if (evt.classList.contains(this.classList[0])) {
        evt.setAttribute('style', 'opacity:0.7; ' + colorFill + ";"); // evt.setAttribute('style', colorFill)
      }
    ...
    let firstType = this.classList[0].split('Arc')[0]; //marchait pas du coup j'ai fait un forEach
    ...
    let typeNumber; //console.log(matrix[1])
...
makeStatsMiniBoxPlot(stats.water, 'water'); // creation tableau simplifié
//top 10
...
makeStatsMiniBoxPlot(underUsedStats, underName); //3 - RU------------------------------------------------------------
...
makeStatsMiniBoxPlot(rarelyUsedStats, rarelyName); //4 - NU------------------------------------------------------------
...
makeStatsMiniBoxPlot(neverUsedStats, neverName); //AFFICHAGE TOP10 ON DOM-------------------------A AMELIORER-------
...
function prepareAndRender(tierName) {
  ...
  let topTenNames = []; //renderTiersOnDom(topTenNames, tierName)
...
function makeStatsByTier(array) {
  ...
  let sp_atk = array.map(function (d) {
    return d["Sp..Atk"];
  }); // y a un dataset qui est mieux que l'autre dans son organisation des donnée quand même...
...
function makeStatsMiniBoxPlot(data, tierName) {
  ...
  var center = 45;
  var widthBox = 20;
  let svgStats = d3.select(tierNode).append("svg").attr("width", 400).attr("height", 300).append("g").attr("transform", "translate(" + 25 + "," + 25 + ") scale(0.7,0.7)"); //chartes des stats sur les tiers 
  // Build and Show the Y scale

  let y = d3.scaleLinear().domain([0, 160]) // Note that here the Y scale is set manually
  .range([height2, 0]);
  svgStats.append("g") //.attr("transform", "translate("+ width2 +" 0)")
  .call(d3.axisLeft(y).tickValues(["20", "40", "60", "80", "100", "120", "140", "160"])).attr("class", "graphLines");
  ...
  let x = d3.scaleBand().range([0, width2]).domain(["HP", "ATK", "DEF", "SP.ATK", "SP.DEF", "SPD"]).padding(0.05); // This is important: it is the space between 2 groups. 0 means no padding. 1 is the maximum.

  svgStats.append("g").attr("transform", "translate(0," + height2 + ")").call(d3.axisBottom(x));
  let hpData = data["HP"];
  ...
  let allStatsData = [hpData, atkData, defData, spAtkData, spDef, spdData];
  ...
  function makeBars(data, svgStats, colorCode) {
    ...
    let data_sorted = data.sort(d3.ascending); // console.log(data_sorted)
    ...
    let max = q1 + 1.5 * interQuantileRange; // svgStats
    // .append("line")
    //   .attr("x1", center)
    //   .attr("x2", center)
    //   .attr("y1", y(min) )
    ...
    svgStats.append("rect").attr("x", center - widthBox / 2).attr("y", y(q3)).attr("height", y(q1) - y(q3)).attr("width", widthBox).attr("stroke", "white").attr("rx", "5").style("fill", () => {
      if (colorCode != null) {
        return colorCode;
      } else {
        return "#a6a6a6";
    ...
    svgStats.selectAll("lines").data([median]).enter().append("line").attr("x1", center - widthBox / 2).attr("x2", center + widthBox / 2).attr("y1", function (d) {
      return y(d);
    }).attr("y2", function (d) {
      return y(d);
...
let barsIndex = 0; //fonctionne mais wonky... et les fait toutes a la meme place vu que var de
...
function makeMiniCharts(matrix, tierName) {
  ...
  let svgTier = d3.select(tierNode).append("svg").attr("width", 400).attr("height", 400).append("g").attr("transform", "translate(200,150) scale(0.7,0.7)"); //calcul de la matrice

  let resTier = d3.chord().padAngle(0.03).sortSubgroups(d3.descending).sortChords(d3.descending)(matrix); // Groupes dans la partie exterieur du cercle

  let outerGroups = svgTier.datum(resTier).append("g").selectAll("g").data(function (d) {
    return d.groups;
  }).enter().append("g");
  let outerBarsTier = outerGroups.attr('class', "groupTier").attr('type', function (d) {
    return typeArray[d.index];
  }).append("path").style("fill", function (d, i) {
    return colors[i];
  }).style("stroke", function (d, i) {
    return colors[i];
  }).attr("d", d3.arc().innerRadius(200).outerRadius(210)); //append  des elements g

  let outerTextTier = outerBarsTier.data(resTier.groups).enter().append("svg:g").attr("class", function (d) {
    return "groupTier " + typeArray[d.index];
  }).append("svg:textPath").text(function (d) {
    return typeArray[d.index];
  }); // Ajout des liens entre les groupes-------

  let innerBarsTier = svgTier.datum(resTier).append("g").selectAll("path").data(function (d) {
    return d;
  }).enter().append("path").attr('class', 'innerArcs').on("mouseover", onMouseOver).on("mouseout", onMouseOut) // .on("mouseout", onMouseOut)
  .attr("d", d3.ribbon().radius(200)).attr("id", function (d) {
    return [d.type1];
  }) //ICIIIII 
  ...
  function onMouseOver(selected) {
    // console.log(this)
    const style = getComputedStyle(this); //this.getAttribute('style');

    let color = style.fill;
    let colorFill = "fill:" + color;
    innerBarsTier.style("opacity", 0.1);
    this.setAttribute('style', 'opacity:1');
    this.setAttribute('style', colorFill);
  }
...
function makeLegend(typeArray, colors) {
  ...
  let preventFirstLabel = 0; // create a list of keys

  let svg = d3.select('#legendBullets');
  let color = d3.scaleOrdinal().domain(keys).range(colors);
  svg.selectAll("mydots").data(typeArray).enter().append("circle").attr("cx", function (d, i) {
    //console.log(i)
    let position = 50;
  ...
  svg.selectAll("mylabels").data(keys).enter().append("text").attr("x", function (d, i) {
    //console.log(i)
    let position = 120;
